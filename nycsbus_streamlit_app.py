...
def coerce_latlon(df: pd.DataFrame) -> pd.DataFrame:
    if "latitude" in df.columns and "longitude" in df.columns:
        ...
        return df
    lat_candidates = [c for c in df.columns if c in {"lat","latitude","center_lat"}]
    lon_candidates = [c for c in df.columns if c in {"lon","lng","longitude","center_lon"}]
...
def make_folium_map(df_points: pd.DataFrame,
                    ...
                    zoom=10.5,
                    center=NYC_CENTER):
    # This function is only called if FOLIUM_AVAILABLE is True
    ...
    for _, r in df_points.dropna(subset=[lat_col, lon_col]).iterrows():
        ...
        folium.CircleMarker(
            ...
            radius=5, weight=1, fill=True, fill_opacity=0.85,
            color="#cccccc", fill_color="#dddddd"
...
boro_present = sorted([b for b in BOROUGH_ORDER if "borough" in alarms_df.columns and (alarms_df["borough"] == b).any()]) \
    or sorted([str(x) for x in alarms_df.get("borough", pd.Series(dtype=str)).dropna().unique()]) \
    or BOROUGH_ORDER
...
sel_boroughs = st.sidebar.multiselect("Boroughs", options=boro_present, default=boro_present)
